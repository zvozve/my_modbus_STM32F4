...
//    USR_UART_Print_STA();
    switch (mbMaster_Task.status) {
        ...
        case MBT_REQUESTING:
            ...
            USR_UART_Transmit_DMA(&huart_mbm);
        break;
        // 发送完成: 打开接收，开始计时
        case MBT_TX_CPLT:
            USR_UART_Receive_DMA(&huart_mbm);
...
void mbSlave_Poll(void) {
    ...
    switch (mbSlave_Task.status) {
        // 初始化并打开串口空闲接收
        case MBT_IDLE:
            USR_UART_Receive_DMA(&huart_mbs);
        break;

        case MBT_RX_ING:
            if (USR_UART_Unstable(&huart_mbs)) {
        ...
        case MBT_TX_ENTRY:
            USR_UART_Transmit_DMA(&huart_mbs);
        break;
