 ...
 * 多实例测试时默认关掉 server（MB_TCP_CLIENT_TEST_DISABLE_SERVER=1）以腾出
 * ~6.9KB 给 client；如需 server 与 client 并存，把该宏设 0 并把 COUNT 限到 1。
 * ============================================================ */
#ifndef MB_TCP_CLIENT_COUNT
#define MB_TCP_CLIENT_COUNT   2      /* 注册的 client 实例数（按需调大，受 RAM 限制） */
#endif
#ifndef MB_TCP_CLIENT_TEST_DISABLE_SERVER
#define MB_TCP_CLIENT_TEST_DISABLE_SERVER  1   /* 1=测试期间关掉 server 省 RAM；0=server 并存(限 1 client) */
#endif
